...
beforeEach(() => {
  store = new FlowStore();
  el = document.createElement("flow-canvas");
  el.store = store;
...
describe("FlowCanvas", () => {
  it("renders a flow-node when a node is added to the store", async () => {
    ...
    expect(el.shadowRoot.querySelector("flow-node")).toHaveProperty("nodeType", "Delay");
  ...
  it("selects a node when it is selected in the store", async () => {
    ...
    await el.updateComplete;
    const nodeEls = [...el.shadowRoot.querySelectorAll("flow-node")];
  ...
  it("updates the store when a flow-node is selected", async () => {
    const nodeId = store.addNode("Delay", 6, 0);
    await el.updateComplete;
    const flowNodeEl = el.shadowRoot.querySelector("flow-node");
